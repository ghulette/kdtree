...
void kdt_range_search_rec(tree_node *root, point *min, point *max, point_list *result, int depth) {
	...
	int axis = depth % KDT_DIM;
	//fprintf(stderr, "Testing (%0.2f, %0.2f, %0.2f) on axis %d\n", root->loc->coord[0], root->loc->coord[1], root->loc->coord[2], axis);
...
void kdt_range_search(tree_node *root, point *min, point *max, point_list *result) {
	//printf("Searching inside rect [(%0.2f, %0.2f, %0.2f), (%0.2f, %0.2f, %0.2f)]\n", min->coord[0], min->coord[1], min->coord[2], max->coord[0], max->coord[1], max->coord[2]);
...
int kdt_collision_detect(tree_node *root, point *start, point *end, point *c, float epsilon, point_list *pts) {
	...
	for(i=0; i < pts->num_points; i++) {
		...
		//printf("Testing point %0.2f, %0.2f, %0.2f\n", pt->coord[0], pt->coord[1], pt->coord[2]);
		vec_sub(&b, pt, start);
		// Collisions should be ordered, with the lowest xhat value being the first
		// currently the last collision detected is returned, with detections in random order
...
int kdt_print_tree_with_id(tree_node *root, int id_num) {
	...
	printf("\t%d [label=\"%0.2f, %0.2f, %0.2f\"]\n", id_num, root->loc->coord[0], root->loc->coord[1], root->loc->coord[2]);
